...
%% evaluation on Vid4
addpath('metrics')
video_name = dir('D:/results/16to9/sof/BI_x4/*');
...
for idx_video = 1:length(video_name)
    ...
    for idx_frame = 2:30 				% exclude the first and last 2 frames
        srname = ['D:/results/16to9/sof/BI_x4/' video_name(idx_video).name '/sr_' num2str(idx_frame,'%02d') '.png'];
...
disp(['---------------------------------------------'])
disp(['SOF ',degradation,'_x', num2str(scale) ,' SR---Mean PSNR: ', num2str(mean(psnr_vid4),'%0.4f'),', Mean SSIM: ', num2str(mean(ssim_vid4),'%0.4f')])
fprintf(file, ['SOF ',degradation,'_x', num2str(scale) ,' SR---Mean PSNR: ', num2str(mean(psnr_vid4),'%0.4f'),', Mean SSIM: ', num2str(mean(ssim_vid4),'%0.4f'), '\n\n']);
fclose(file);